...
import MSMEList from "@/components/superadmin/MSMEList";
import GuestPagePagination from "@/components/guest/GuestPagePagination";
  ...
  const displayedMSME = useMemo(() => {
    ...
    if (selectedSector.length > 0) {
      filtered = filtered.filter((msme) =>
        selectedSector.includes(msme.sectorName),
      );
    }
